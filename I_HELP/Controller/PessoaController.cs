    ...
    public class PessoaController
    {
        ...
        public bool GravarPessoa(PessoaEntity pessoa)
        {
            ...
            using (banco = new BancoInstance())
            {
                ...
                bool ok = false;
                ok = banco.Banco.ExecuteNonQuery(@"insert into pessoa (nompes, cpfpes, rgpes, telpes, naspes, codfam) values(@1, @2, @3, @4, @5, @6)", "@1", pessoa.Nome, "@2", pessoa.Cpf, "@3", pessoa.Rg, "@4", pessoa.Telefone, "@5", formattedDate, "@6", pessoa.Codfam);
        ...
        public DataTable BuscarPessoaId(int id)
        {
            DataTable objResultado = new DataTable();
            BancoInstance banco;
            using (banco = new BancoInstance())
            {
                banco.Banco.ExecuteQuery(@"select * from pessoa where codpes=@1", out objResultado, "@1", id);
                return objResultado;
            }
        }
        // Excluir Pessoa
        ...
        public DataTable SelecionarPessoas()
        {
            BancoInstance banco;
